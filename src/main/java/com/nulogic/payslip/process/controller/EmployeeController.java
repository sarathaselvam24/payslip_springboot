...
public class EmployeeController {
	...
	public ResponseEntity<EmployeeAccountDetails> getEmployeeAccountDetails(@RequestBody Map<String, String> request) {
		...
		return employeeAccountData.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
	}
	...
	public ResponseEntity<EmployeeAccountDetails> saveEmployeeAccountDetails(
			@RequestBody EmployeeAccountDetails accountDetails) {
...
//    }
	...
	@PostMapping("/api/employeeSalaryDetails")
	public ResponseEntity<ByteArrayResource> getEmployeePayslip(Model model, @RequestBody Map<String, String> request)
	...
	public ResponseEntity<Boolean> saveLoanRequest(@RequestBody Map<String, String> request) {
		...
		if (!onGoingLoanRequest.isEmpty()) {
			...
			if (ctcrepo != null) {
				...
				loanRequest.setBasicpay(ctcrepo.getCtc()
	...
	public ResponseEntity<List<Loan>> employeeLoanRequestByEmpid(@RequestBody Map<String, String> request) {
		...
		return employeeLoanRequests.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
	...
	public ResponseEntity<List<Loan>> searchMyLoanRequestByStatus(@RequestBody Map<String, String> request) {
		...
		if (search == null || search.isEmpty()) {
			...
			return employeeAllLoanRequests.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
		} else {
	...
	public ResponseEntity<List<Loan>> searchEmployeeLoanRequestByStatusOrEmpid(
		...
		if (search == null || search.isEmpty()) {
			...
			Optional<List<Loan>> employeeLoanRequestsByStatusOrEmpid = loanRepo
					.findByEmpidContainingOrLoanstatusContaining(search, search);
			System.out
					.println("employeeLoanRequestsByStatusOrEmpid else called " + employeeLoanRequestsByStatusOrEmpid);
	...
	public ResponseEntity<List<Loan>> viewEmployeeLoanRequest() {
		...
		return loanreq.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
	}
	...
	public ResponseEntity<String> generatePaySlipinDB(@RequestBody Map<String, String> request) {
		...
		for (Basicpay employee : emp) {
			...
			if (sal != null && !sal.isEmpty()) {
				...
				BigDecimal houseAllowance = employee.getCtc()
				...
				houseAllowance = houseAllowance.divide(BigDecimal.valueOf(12), MathContext.DECIMAL128).setScale(2,
						RoundingMode.HALF_UP);
				BigDecimal specialAllowance = employee.getCtc()
				...
				details = new Salarydetails(sal.get().getEmpid(), sal.get().getBasicpay(),
						sal.get().getHouseallowance(), sal.get().getSpecialallowance(), nightshift, providentFund,
						professionalTex, salaryadvance, BigDecimal.ZERO, payabledays, paiddays, month, year, overtime);
				...
				specialAllowance = specialAllowance.divide(BigDecimal.valueOf(12), MathContext.DECIMAL128).setScale(2,
						RoundingMode.HALF_UP);
				Overtime otime = overTimeRepo.exitsOverTime(month, year, employee.getEmpid());
				BigDecimal overtime = BigDecimal.ZERO;
				if (otime != null) {
					overtime = otime.getOvertime();
				}

				Loan loanNotStartedReq = loanRepo.findLoanNotStaredrequest(employee.getEmpid(), "Approved",
						"Not Started");
				...
				if (loanNotStartedReq != null) {
					...
					System.out
							.println("formatMonthAndYear(month+\" \"+year) " + formatMonthAndYear(month + " " + year));

					int compareDate = loanNotStartedReq.getEmistartsfrom()
							.compareTo(formatMonthAndYear(month + " " + year));
				...
				Loan loanReq = loanRepo.findLoanrequest(employee.getEmpid(), "Approved", "On going",
				...
				if (loanReq != null) {
					...
					if (loanrequestRemainingBalance == BigDecimal.valueOf(0) || ((loanReq.getLoanamount()
							.subtract(loanReq.getEmi().multiply(loanReq.getRepaymentterms())))
							.compareTo(loanReq.getRemainingbalance()) == 0)) {
